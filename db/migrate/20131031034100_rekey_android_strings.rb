class RekeyAndroidStrings < ActiveRecord::Migration
  ...
  def cached_blob_sha(project, revision, path)
    @blob_cache                             ||= Hash.new
    @blob_cache[project.id]                 ||= Hash.new
    @blob_cache[project.id][revision]       ||= Hash.new
    @blob_cache[project.id][revision][path] ||= project.repo.object("#{revision}^{tree}:#{path}").sha
  end

  def cached_blob_xml(project, sha)
    @blob_cache                  ||= Hash.new
    @blob_cache[project.id]      ||= Hash.new
    @blob_cache[project.id][sha] ||= Nokogiri::XML(project.repo.object(sha).contents)
  end
end
